...
def ln_divide_FRC(eptm, edge):
    ...
    new_length_elasticity_FRC = eptm.edge_df.loc[edge, 'length_elasticity_FRC'] / 3
...
def ln_divide_one_FRC(sheet, condition, energyContributions_model):
    ...
    division_edge = filtered_edges[filtered_edges[condition] == filtered_edges[condition].max()]

    longest_edge_id = random.choice(division_edge.index)
